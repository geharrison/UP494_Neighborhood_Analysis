...
```{r}
ex_list2<-c("I", "am", "going", "to", "the", "store", "tomorrow")
ex_list2[c(7, 4, 5, 6, 1, 2, 3)]
```
...
```{r}
oatmeal$instructions<- with(oatmeal, paste0(quantity45, units, ingredients))
...
For logical values, R codes 1 as TRUE and 0 as false, meaning if we wanted to count the number of undesignated tracts, we could simply ask for the sum of the values for which the logical test is true (the sum of the values that are `NA`):
```{r}
...
mean(ozs$vacancyrate[ozs$state == "Illinois" & ozs$Designated == 0])

```

3. Think of another question you'd like to ask of these data - write it down and work the problem out.

Report the average household income for designated opportunity zones in Indiana.

```{r}
mean(ozs$medhhincome2014_tract[ozs$Designated == 1 & ozs$state == "Indiana"])
```
